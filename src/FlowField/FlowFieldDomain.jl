...
struct FlowFieldDomain{T<:Real}
    ...
    function FlowFieldDomain(Nx::Int, Ny::Int, Nz::Int, num_dimensions::Int,
        Lx::T, Lz::T, a::T, b::T) where {T<:Real}
        # Validate inputs
...
function Base.:(==)(d1::FlowFieldDomain, d2::FlowFieldDomain)
    return (d1.Nx == d2.Nx && d1.Ny == d2.Ny && d1.Nz == d2.Nz &&
            ...
            d1.Lx ≈ d2.Lx && d1.Lz ≈ d2.Lz && d1.a ≈ d2.a && d1.b ≈ d2.b)
end
...
function geom_congruent(d1::FlowFieldDomain, d2::FlowFieldDomain; eps::Real=1e-13)
    return (d1.Nx == d2.Nx && d1.Ny == d2.Ny && d1.Nz == d2.Nz &&
            abs(d1.Lx - d2.Lx) < eps && abs(d1.Lz - d2.Lz) < eps &&
            abs(d1.a - d2.a) < eps && abs(d1.b - d2.b) < eps)
end
...
function y_coord(domain::FlowFieldDomain, ny::Int)
    ...
    return 0.5 * (domain.a + domain.b) + 0.5 * (domain.b - domain.a) * cos(π * (ny - 1) / (domain.Ny - 1))
end
...
function x_gridpoints(domain::FlowFieldDomain)
    return [x_coord(domain, nx) for nx in 1:domain.Nx]
end
...
function y_gridpoints(domain::FlowFieldDomain)
    return [y_coord(domain, ny) for ny in 1:domain.Ny]
end
...
function z_gridpoints(domain::FlowFieldDomain)
    return [z_coord(domain, nz) for nz in 1:domain.Nz]
end
...
function kx_range(domain::FlowFieldDomain)
    return [kx for kx in (-div(domain.Nx, 2)+1):div(domain.Nx, 2)]
end
...
function kz_range(domain::FlowFieldDomain)
    return [kz for kz in 0:(domain.Mz-1)]
end
