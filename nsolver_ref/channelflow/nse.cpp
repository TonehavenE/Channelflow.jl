...
void flowfield_print(FlowField ff) {
  printf("FlowField:\n");
  printf("  Domain: [0, %f] x [%f, %f] x [0, %f]\n", ff.Lx(), ff.a(), ff.b(),
         ff.Lz());
  printf("  XZ State: %d\n", ff.xzstate());
  printf("  Y State: %d\n", ff.ystate());
  printf("  Vector Dimension: %d\n", ff.vectorDim());

  if (ff.xzstate() == Spectral) {
    printf("  State: Spectral (printing coefficients)\n");
    for (int i = 0; i < ff.vectorDim(); ++i) {
      for (int mx = 0; mx < ff.Mx(); ++mx) {
        for (int my = 0; my < ff.My(); ++my) {
          for (int mz = 0; mz < ff.Mz(); ++mz) {
            printf("  i = %d mx=%d my=%d mz=%d: %8.3f + %8.3f i\n", i, mx, my,
                   mz, real(ff.cmplx(mx, my, mz, i)),
                   imag(ff.cmplx(mx, my, mz, i)));
          }
        }
      }
    }
  } else {
    printf("  State: Physical (printing values)\n");
    for (int i = 0; i < ff.vectorDim(); ++i) {
      for (int nx = 0; nx < ff.Nx(); ++nx) {
        for (int ny = 0; ny < ff.Ny(); ++ny) {
          for (int nz = 0; nz < ff.Nz(); ++nz) {
            printf("  i = %d nx=%d ny=%d nz=%d: %8.3f\n", i, nx, ny, nz,
                   ff(nx, ny, nz, i));
          }
        }
      }
...
void navierstokesNL(const FlowField &u_, ChebyCoeff Ubase, ChebyCoeff Wbase,
                    FlowField &f, FlowField &tmp, DNSFlags &flags) {
  ...
  if (flags.nonlinearity == LinearAboutProfile)
    ...
    // add rotation term -(omega x u) = - flags.rotation * flags.nu (y e_y + e_z
    // x u)
    if (flags.rotation != 0.0) {
      u.makePhysical();
      lint Nz = f.Nz();
...
NSE::NSE()
    : lambda_t_(0), tausolver_(0), flags_(), taskid_(0),
      // Spatial parameter members
      nxlocmin_(0), Nxloc_(0), nylocmin_(0), nylocmax_(0), Nz_(0), mxlocmin_(0),
      Mxloc_(0), My_(0), mzlocmin_(0), Mzloc_(0), Nyd_(0), kxd_max_(0),
      kzd_max_(0), Lx_(0), Lz_(0), a_(0), b_(0), kxmax_(0), kzmax_(0),
      kxloc_(0), kzloc_(0),
      // Base flow members
      dPdxRef_(0), dPdxAct_(0), dPdzRef_(0), dPdzAct_(0), UbulkRef_(0),
      UbulkAct_(0), UbulkBase_(0), WbulkRef_(0), WbulkAct_(0), WbulkBase_(0),
      Ubase_(), Ubaseyy_(), Wbase_(), Wbaseyy_(),
      // Memspace members
      tmp_(), uk_(), vk_(), wk_(), Pk_(), Pyk_(), Ruk_(), Rvk_(), Rwk_() {}

NSE::NSE(const NSE &nse)
    : lambda_t_(nse.lambda_t_), tausolver_(0), flags_(nse.flags_),
      taskid_(nse.taskid()),
      // Spatial parameter members
      nxlocmin_(nse.nxlocmin_), Nxloc_(nse.Nxloc_), nylocmin_(nse.nylocmin_),
      nylocmax_(nse.nylocmax_), Nz_(nse.Nz_), mxlocmin_(nse.mxlocmin_),
      Mxloc_(nse.Mxloc_), My_(nse.My_), mzlocmin_(nse.mzlocmin_),
      Mzloc_(nse.Mzloc_), Nyd_(nse.Nyd_), kxd_max_(nse.kxd_max_),
      kzd_max_(nse.kzd_max_), Lx_(nse.Lx_), Lz_(nse.Lz_), a_(nse.a_),
      b_(nse.b_), kxmax_(nse.kxmax_), kzmax_(nse.kzmax_), kxloc_(nse.kxloc_),
      kzloc_(nse.kzloc_),
      // Base flow members
      dPdxRef_(nse.dPdxRef_), dPdxAct_(nse.dPdxAct_), dPdzRef_(nse.dPdzRef_),
      dPdzAct_(nse.dPdzAct_), UbulkRef_(nse.UbulkRef_),
      UbulkAct_(nse.UbulkAct_), UbulkBase_(nse.UbulkBase_),
      WbulkRef_(nse.WbulkRef_), WbulkAct_(nse.WbulkAct_),
      WbulkBase_(nse.WbulkBase_), Ubase_(nse.Ubase_), Ubaseyy_(nse.Ubaseyy_),
      Wbase_(nse.Wbase_), Wbaseyy_(nse.Wbaseyy_),
      // Memspace members
      tmp_(nse.tmp_), uk_(nse.uk_), vk_(nse.vk_), wk_(nse.wk_), Pk_(nse.Pk_),
      Pyk_(nse.Pyk_), Ruk_(nse.Ruk_), Rvk_(nse.Rvk_), Rwk_(nse.Rwk_) {
...
NSE::NSE(const vector<FlowField> &fields, const DNSFlags &flags)
    : lambda_t_(0), tausolver_(0), // tausolvers are allocated when reset_lambda
                                   // is called for the first time
      flags_(flags), taskid_(fields[0].taskid()),
      // Spatial parameter members
      nxlocmin_(fields[0].nxlocmin()), Nxloc_(fields[0].Nxloc()),
      nylocmin_(fields[0].nylocmin()), nylocmax_(fields[0].nylocmax()),
      Nz_(fields[0].Nz()), mxlocmin_(fields[0].mxlocmin()),
      Mxloc_(fields[0].Mxloc()), My_(fields[0].Ny()),
      mzlocmin_(fields[0].mzlocmin()), Mzloc_(fields[0].Mzloc()),
      Nyd_(flags.dealias_y() ? 2 * (fields[0].numYmodes() - 1) / 3 + 1
                             : fields[0].numYmodes()),
      kxd_max_(flags.dealias_xz() ? fields[0].Nx() / 3 - 1 : fields[0].kxmax()),
      kzd_max_(flags.dealias_xz() ? fields[0].Nz() / 3 - 1 : fields[0].kzmax()),
      Lx_(fields[0].Lx()), Lz_(fields[0].Lz()), a_(fields[0].a()),
      b_(fields[0].b()), kxmax_(fields[0].kxmax()), kzmax_(fields[0].kzmax()),
      kxloc_(0), kzloc_(0),
      // Base flow members
      dPdxRef_(0), dPdxAct_(0), dPdzRef_(0), dPdzAct_(0), UbulkRef_(0),
      UbulkAct_(0), UbulkBase_(0), WbulkRef_(0), WbulkAct_(0), WbulkBase_(0),
      Ubase_(), Ubaseyy_(), Wbase_(), Wbaseyy_(),
      // Memspace members
      tmp_(), uk_(Nyd_, a_, b_, Spectral), vk_(Nyd_, a_, b_, Spectral),
      wk_(Nyd_, a_, b_, Spectral), Pk_(Nyd_, a_, b_, Spectral),
      Pyk_(Nyd_, a_, b_, Spectral), Ruk_(Nyd_, a_, b_, Spectral),
      Rvk_(Nyd_, a_, b_, Spectral), Rwk_(Nyd_, a_, b_, Spectral) {
  ...
  if (flags_.nonlinearity == Alternating ||
    ...
    tmp_.resize(fields[0].Nx(), fields[0].Ny(), fields[0].Nz(), 9,
                fields[0].Lx(), fields[0].Lz(), fields[0].a(), fields[0].b(),
                fields[0].cfmpi());
  } else
    tmp_.resize(fields[0].Nx(), fields[0].Ny(), fields[0].Nz(), 3,
                fields[0].Lx(), fields[0].Lz(), fields[0].a(), fields[0].b(),
                fields[0].cfmpi());
...
NSE::NSE(const vector<FlowField> &fields, const vector<ChebyCoeff> &base,
    ...
    :                              //   fields_(fields),
      ...
      dPdxRef_(0), dPdxAct_(0), dPdzRef_(0), dPdzAct_(0), UbulkRef_(0),
      UbulkAct_(0), UbulkBase_(0), WbulkRef_(0), WbulkAct_(0), WbulkBase_(0),
      Ubase_(base[0]), Ubaseyy_(), Wbase_(base[1]), Wbaseyy_(),
      // Memspace members
      tmp_(), uk_(Nyd_, a_, b_, Spectral), vk_(Nyd_, a_, b_, Spectral),
      wk_(Nyd_, a_, b_, Spectral), Pk_(Nyd_, a_, b_, Spectral),
      Pyk_(Nyd_, a_, b_, Spectral), Ruk_(Nyd_, a_, b_, Spectral),
...
void NSE::nonlinear(const vector<FlowField> &infields,
                    vector<FlowField> &outfields) {
  // The first entry in vector must be velocity FlowField, only use this entry
  // for NLT calculation. Pressure as second entry in in/outfields is not
  // touched. Dealiasing must be done separately, e.g. by calling nse::solve
...
void NSE::linear(const vector<FlowField> &infields,
                 vector<FlowField> &outfields) {
  // Method takes input fields {u,press} and computes the linear terms for
  // velocity output field {u}

  assert(infields.size() ==
         (outfields.size() +
          1)); // Make sure user does not expect a pressure output. Outfields
  ...
  // is most sufficient on ComplexChebyCoeff. Therefore, the old loop structure
  // is kept.
  for (lint mx = mxlocmin_; mx < mxlocmin_ + Mxloc_; ++mx) {
    ...
    for (lint mz = mzlocmin_; mz < mzlocmin_ + Mzloc_; ++mz) {
      ...
      // (3) Summation of all derivative terms and assignment to output velocity
      // field.
      const Real kappa2 = 4 * pi * pi * (square(kx / Lx_) + square(kz / Lz_));
      const Complex Dx = infields[0].Dx(mx);
      const Complex Dz = infields[0].Dz(mz);
      for (int ny = 0; ny < Nyd_; ++ny) {
        outfields[0].cmplx(mx, ny, mz, 0) =
            Ruk_[ny] - kappa2 * uk_[ny] - Dx * Pk_[ny];
        outfields[0].cmplx(mx, ny, mz, 1) =
            Rvk_[ny] - kappa2 * vk_[ny] - Pyk_[ny];
        outfields[0].cmplx(mx, ny, mz, 2) =
            Rwk_[ny] - kappa2 * wk_[ny] - Dz * Pk_[ny];
      }
      ...
      if (kx == 0 && kz == 0) {
        ...
        if (Ubaseyy_.length() > 0)
          for (int ny = 0; ny < My_; ++ny)
            outfields[0].cmplx(mx, ny, mz, 0) +=
                Complex(flags_.nu * Ubaseyy_[ny], 0);
        if (Wbaseyy_.length() > 0)
          for (int ny = 0; ny < My_; ++ny)
            outfields[0].cmplx(mx, ny, mz, 2) +=
                Complex(flags_.nu * Wbaseyy_[ny], 0);
...
void NSE::solve(vector<FlowField> &outfields, const vector<FlowField> &rhs,
                const int s) {
  // Method takes a right hand side {u} and solves for output fields {u,press}

  assert(outfields.size() ==
         (rhs.size() + 1)); // Make sure user provides correct RHS which can be
                            // created outside NSE with NSE::createRHS()
  const int kxmax = outfields[0].kxmax();
  const int kzmax = outfields[0].kzmax();

  // printf("outfields[0] is:\n");
  // flowfield_print(outfields[0]);
  // printf("rhs[0] is:\n");
  // flowfield_print(rhs[0]);
    ...
    for (lint iz = 0; iz < Mzloc_; ++iz) {
      ...
      // 		solve(ix,iz,uk_,vk_,wk_,Pk_, Ruk_,Rvk_,Rwk_);
        ...
        if (Ubaseyy_.length() > 0)
          for (int ny = 0; ny < My_; ++ny)
            Ruk_.re[ny] +=
                flags_.nu * Ubaseyy_[ny]; // Rx has addl'l term from Ubase
        if (Wbaseyy_.length() > 0)
          for (int ny = 0; ny < My_; ++ny)
            Rwk_.re[ny] +=
                flags_.nu * Wbaseyy_[ny]; // Rz has addl'l term from Wbase
        ...
        if (flags_.constraint == PressureGradient) {
          ...
          tausolver_[s][ix][iz].solve(uk_, vk_, wk_, Pk_, Ruk_, Rvk_, Rwk_);
          // 	  	solve(ix,iz,uk_, vk_, wk_, Pk_, Ruk_,Rvk_,Rwk_);
          // Bulk vel is free variable determined from soln of tau eqn //TODO:
          // write method that computes UbulkAct everytime it is needed
          ...
          tausolver_[s][ix][iz].solve(uk_, vk_, wk_, Pk_, dPdxAct_, dPdzAct_,
                                      Ruk_, Rvk_, Rwk_, UbulkRef_ - UbulkBase_,
          ...
          assert((UbulkRef_ - UbulkBase_ - uk_.re.mean()) <
                 1e-15); // test if UbulkRef == UbulkAct = UbulkBase_ +
                         // uk_.re.mean()
          assert((WbulkRef_ - WbulkBase_ - wk_.re.mean()) <
                 1e-15); // test if WbulkRef == WbulkAct = WbulkBase_ +
                         // wk_.re.mean()
        }
      }
      ...
      if ((kx == 0 && kz == 0) ||
          (outfields[0].Nx() % 2 == 0 && kx == kxmax && kz == 0) ||
          (outfields[0].Nz() % 2 == 0 && kz == kzmax && kx == 0) ||
          (outfields[0].Nx() % 2 == 0 && outfields[0].Nz() % 2 == 0 &&
           kx == kxmax && kz == kzmax)) {
        for (int ny = 0; ny < Nyd_; ++ny) {
...
vector<FlowField> NSE::createRHS(const vector<FlowField> &fields) const {
  return {fields[0]};
}
...
void NSE::createCFBaseFlow() {
  ...
  case ParabolicBase:
    ...
    break;
  case SuctionBase:
    Ubase_ = laminarProfile(flags_.nu, PressureGradient, 0, flags_.Ubulk,
                            flags_.Vsuck, a_, b_, -0.5, 0.5, My_);
    Wbase_ = ChebyCoeff(My_, a_, b_, Spectral);
    break;
  case LaminarBase:
    Ubase_ =
        laminarProfile(flags_.nu, flags_.constraint, flags_.dPdx, flags_.Ubulk,
                       flags_.Vsuck, a_, b_, ulowerwall, uupperwall, My_);

    Wbase_ =
        laminarProfile(flags_.nu, flags_.constraint, flags_.dPdz, flags_.Wbulk,
                       flags_.Vsuck, a_, b_, wlowerwall, wupperwall, My_);
    break;
  case ArbitraryBase:
  ...
  default:
    cerr << "error in NSE::createBaseFlow :\n";
    cerr << "flags.baseflow should be ZeroBase, LinearBase, ParabolicBase, "
            "LaminarBase, SuctionBase.\n";
    cerr << "Other cases require use of the DNS::DNS(fields, base, flags) "
            "constructor.\n";
    cferror("");
  }
}
  ...
  for (uint j = 0; j < lambda_t.size(); ++j) {
    for (int mx = 0; mx < Mxloc_; ++mx) {
      ...
      for (int mz = 0; mz < Mzloc_; ++mz) {
        ...
        if ((kx != kxmax_ || kz != kzmax_) &&
            (!flags_.dealias_xz() || !isAliasedMode(kx, kz)))

          tausolver_[j][mx][mz] =
              TauSolver(kx, kz, Lx_, Lz_, a_, b_, lambda, flags_.nu, Nyd_,
...
bool NSE::isAliasedMode(int kx, int kz) const {
  return (abs(kx) > kxd_max_ || (abs(kz) > kzd_max_)) ? true : false;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////

Real viscosity(Real Reynolds, VelocityScale vscale, MeanConstraint constraint,
               Real dPdx, Real Ubulk, Real Uwall, Real h) {
  Real nu = 0.0;
  /*****************************************************
  cout << "viscosity(Reynolds, vscale, contraint, dPdx, Ubulk, Uwall, h) : " <<
  endl; cout << "  Reynolds == " << Reynolds << endl; cout << "    vscale == "
  << vscale << endl; cout << "constraint == " << constraint << endl; cout << "
  dPdx == " << dPdx << endl; cout << "     Ubulk == " << Ubulk<< endl; cout << "
  Uwall == " << Uwall << endl; cout << "         h == " << h << endl;
  *****************************************************/
  if (vscale == WallScale) {
    ...
    if (constraint == PressureGradient) {
      ...
      // Pressure gradient determines Ubulk, so determine Ucenter and nu as
      // follows U(y) == Ucenter (1-(y/h)^2), so 0 == -dPdx + nu U'' gives
      ...
      // Ucenter == 3/2 Ubulk, so Reynolds == Ucenter h/nu gives nu == 3/2 Ubulk
      // h/Reynolds
...
ChebyCoeff laminarProfile(Real nu, MeanConstraint constraint, Real dPdx,
                          Real Ubulk, Real Vsuck, Real a, Real b, Real ua,
                          Real ub, int Ny) {
  ...
  // For Vsuck != 0, but as H Vsuck/nu -> 0, we evaluate the exponential ASBL
  // solution with a Taylor expansion in mu = Vsuck H/nu in order to avoid loss
  // of precision.

  // PCF/PPF == plane Couette flow/plane Poiseuille flow, ASBL == asympotic
  ...
  if (abs(mu) == 0.0) { // PPF/PCF solution
    ...
    Real H = b - a;
    Real mu = H * Vsuck /
              nu; // dimensionless constant representing magnitude of Vsuck
    Real ub_ua = ub - ua;
    ...
    if (abs(mu) > 1e-01) {
      ...
      if (constraint == BulkVelocity) {
        // Match the bulk velocity  constraint by computing the pressure
        // gradient that produces the correct bulk velocity, and then assigning
        // u[i] with the pressure gradient formula below.
        Real k = -1.0 / expm1_mu -
                 1 / mu; // k is bounded away from 1/2, safe for below
      ...
      for (int i = 0; i < Ny; i++) {
        ...
        u[i] = ua + (ub_ua)*expm1_mu_yhat / expm1_mu +
               dPdx_HH_nu * (yhat - expm1_mu_yhat / expm1_mu) / mu;
      }
    } else {
      // ========= use Taylor expansions for pressure-ASBL formulae ============

      // The direct evaluation formula in the above code block become
      // numerically unstable in the limit H Vsuck/nu -> 0. So for small mu = H
      // Vsuck/nu, we use small-mu Taylor expansions for these formula. The
      // Taylor expansions are computed symbolically
      // pressure-asbl-laminar-taylor-expansion.jl (or .ipynb) included in the
      // channelflow distribution. gibson 2020-04-02.

      if (constraint == BulkVelocity) {
        // Match the bulk velocity  constraint by computing the pressure
        // gradient that produces the correct bulk velocity, and then assigning
        // u[i] with the pressure gradient formula below. This is the Taylor
        // expansion in mu of the formula for dPdx_HH_nu in the "direct
        // evaluation" exponential formula above
        ...
        dPdx_HH_nu =
            6 * B +
            mu * (A + mu * (B / 10 + mu2 * (-B / 1400 + mu2 * B / 126000)));
      }
      ...
      for (int i = 0; i < Ny; i++) {
        Real yh =
            (y[i] - a) / H; // yh stands for yhat = (y - a)/(b-a), yh in [0,1]
        Real yh2 = yh * yh;
        Real yh3 = yh2 * yh;
        Real yh4 = yh3 * yh;
        Real yh_1 = yh - 1;

        // Note: We're relying on compiler to hoist some constant terms out of
        // the loop, but it won't really matter if this doesn't happen. This
        // code should be executed once prior to time-stepping.
            ...
            yh + mu * (-one_2 * yh * yh_1 +
                       mu * (one_12 * yh * yh_1 * (2 * yh - 1) +
                             mu * (-one_24 * square(yh * yh_1) +
                                   mu * (one_720 * yh * yh_1 * (2 * yh - 1) *
                                             (3 * yh2 - 3 * yh - 1) +
                                         mu * (-one_1440 * square(yh * yh_1) *
                                                   (2 * yh2 - 2 * yh - 1) +
                                               mu * (one_30240 * yh * yh_1 *
                                                     (2 * yh - 1) *
                                                     (3 * yh4 - 6 * yh3 +
                                                      3 * yh + 1)))))));
...
ChebyCoeff laminarProfile(const DNSFlags &flags, Real a, Real b, int Ny) {
  if (flags.baseflow == SuctionBase)
    return laminarProfile(flags.nu, PressureGradient, 0, flags.Ubulk,
                          flags.Vsuck, a, b, -0.5, 0.5, Ny);
  else
    return laminarProfile(flags.nu, flags.constraint, flags.dPdx, flags.Ubulk,
                          flags.Vsuck, a, b, flags.ulowerwall, flags.uupperwall,
                          Ny);
}

void changeBaseFlow(const ChebyCoeff &ubase0, const FlowField &ufluc0,
                    const FlowField &q0arg, const ChebyCoeff &ubase1,
  ...
  for (int ny = 0; ny < Ny; ++ny)
    for (int nx = 0; nx < Nx; ++nx)
      for (int nz = 0; nz < Nz; ++nz)
        q1(nx, ny, nz, 0) -=
            0.5 * (square(u0(nx, ny, nz, 0)) + square(u0(nx, ny, nz, 1)) +
  ...
  for (int ny = 0; ny < Ny; ++ny)
    for (int nx = 0; nx < Nx; ++nx)
      for (int nz = 0; nz < Nz; ++nz)
        q1(nx, ny, nz, 0) +=
            0.5 * (square(u1(nx, ny, nz, 0)) + square(u1(nx, ny, nz, 1)) +
