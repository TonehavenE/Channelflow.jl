...
DNSAlgorithm::DNSAlgorithm()
    : flags_(), order_(0), numfields_(0), Ninitsteps_(0), t_(0),
      //   cfl_ (0),
      lambda_t_(), nse_(0) {}

DNSAlgorithm::DNSAlgorithm(const DNSAlgorithm &d)
    : flags_(d.flags_), order_(d.order_), numfields_(d.numfields_),
      Ninitsteps_(d.Ninitsteps_), t_(d.t_),
      //   cfl_ (d.cfl_),
      lambda_t_(d.lambda_t_), nse_(d.nse_), symmetries_(d.symmetries_) {}

DNSAlgorithm::DNSAlgorithm(const vector<FlowField> &fields,
                           const shared_ptr<NSE> &nse, const DNSFlags &flags)
    : flags_(flags), order_(0), numfields_(fields.size()), Ninitsteps_(0),
      t_(flags.t0),
      //   cfl_ (0),
      lambda_t_(0), nse_(nse), symmetries_(0) {
...
cfarray<FieldSymmetry> DNSAlgorithm::symmetries(int ifield) const {
  return symmetries_[ifield];
}
...
TimeStepMethod DNSAlgorithm::timestepping() const {
  return flags_.timestepping;
}
...
MultistepDNS::MultistepDNS(const MultistepDNS &dns)
    : DNSAlgorithm(dns),
      ...
      eta_(dns.eta_), alpha_(dns.alpha_), beta_(dns.beta_),
      fields_(dns.fields_), nonlf_(dns.nonlf_),
...
MultistepDNS::MultistepDNS(const vector<FlowField> &fields,
                           const shared_ptr<NSE> &nse, const DNSFlags &flags)
...
void MultistepDNS::advance(vector<FlowField> &fieldsn, int Nsteps) {
  ...
  vector<FlowField> rhs(nse_->createRHS(
      fieldsn)); // Number of fields and number of RHS's can be different
  int len = rhs.size();
  fields_[0] = fieldsn;
  ...
  for (int step = 0; step < Nsteps; ++step) {
    ...
    // The solution is currently stored in fields_[J]. Shift entire fields and
    // nonlf vectors to move it into fields_[0]. Ie shift fields_[J] <-
    // fields_[J-1] <- ... <- fields_[0] <- fields_[J]
    ...
    // printStack();
    //*flags_.logstream << "} Multistep::advance(...) step == " << step << " }"
    //<<endl;
    if (nse_->taskid() == 0) {
  ...
  *flags_.logstream << "     full == " << full() << endl;
  ...
  for (int j = order_ - 1; j >= 0; --j)
    printf("j=%2d t=%5.2f L2(uj)=%13.10f L2(fj)=%13.10f\n", j,
           t_ - j * flags_.dt, L2Norm(fields_[j][0]), L2Norm(nonlf_[j][0]));
  *flags_.logstream << endl;
  *flags_.logstream << "}" << endl;
}
...
bool MultistepDNS::push(const vector<FlowField> &fields) {
  ...
  if (order_ > 1) {
    fields_[1] = fields;
    // navierstokesNL(u_[0], ubase_, Ubase_, f_[0], tmp_, tmp2_,
    // flags_.nonlinearity);
...
RungeKuttaDNS::RungeKuttaDNS(const RungeKuttaDNS &dns)
    : DNSAlgorithm(dns),
      //   lambda_t_(dns.lambda_t_),
      //   nse_ (),
      Nsubsteps_(dns.Nsubsteps_), Qj1_(dns.Qj1_), Qj_(dns.Qj_), A_(dns.A_),
      B_(dns.B_), C_(dns.C_) {
...
RungeKuttaDNS::RungeKuttaDNS(const vector<FlowField> &fields,
                             const shared_ptr<NSE> &nse, const DNSFlags &flags)
...
void RungeKuttaDNS::advance(vector<FlowField> &fields, int Nsteps) {
  ...
  for (int n = 0; n < Nsteps; ++n) {
    for (int j = 0; j < Nsubsteps_; ++j) {
      // Store uj in un during substeps, reflect in notation, might even
      ...
      Qj_[0] -=
          Qj1_[0]; // subtract because navierstokesNL(u) = u grad u = -N(u)
      ...
      for (int l = 0; l < len; ++l) {
        ...
        // printf("DEBUGGING COMBINE TERMS:");
        // printf("rhs[%d] = lt[%d] is:", l, l);
        // my_print(lt[l]);
        // cout << "lambda_t_[j] is " << lambda_t_[j] << endl;
        // cout << "my_print(uj[l]) is " << endl;
        // my_print(uj[l]);
        // cout << B_C << endl;
        // cout << "my_print(Qj_[l]) is " << endl;
        // my_print(Qj_[l]);

        rhs[l].add(lambda_t_[j], uj[l], B_C, Qj_[l]);
        // cout << "\n\n\n====================\n"
        //      << "after add!, rhs[" << l << "] is:" << endl;
        // my_print(rhs[l]);
      }
...
CNABstyleDNS::CNABstyleDNS(const CNABstyleDNS &dns)
    : DNSAlgorithm(dns),
      //   lambda_t_(dns.lambda_t_),
      //   nse_ (),
      Nsubsteps_(dns.Nsubsteps_), full_(dns.full_), fj1_(dns.fj1_),
      fj_(dns.fj_), alpha_(dns.alpha_), beta_(dns.beta_), gamma_(dns.gamma_),
      zeta_(dns.zeta_) {
  // make copy of nse objects
  //   nse_.resize(Nsubsteps_);
  //   for(int j=0; j<Nsubsteps_; ++j){
...
CNABstyleDNS::CNABstyleDNS(const vector<FlowField> &fields,
                           const shared_ptr<NSE> &nse, const DNSFlags &flags)
    : DNSAlgorithm(fields, nse, flags), full_(false), fj1_(fields),
      fj_(fields) {
  TimeStepMethod algorithm = flags_.timestepping;
  switch (algorithm) {
...
void CNABstyleDNS::reset_dt(Real dt) {
  ...
  // For CNAB2, need to take one forward Euler step to initialize u and f
  // cfarrays.
...
void CNABstyleDNS::project() {
  ...
  // project(symm, fj1_); // no need; gets overwritten w fj_ first thing in
  // advance
}
...
void CNABstyleDNS::advance(vector<FlowField> &fields, int Nsteps) {
  ...
  for (int n = 0; n < Nsteps; ++n) {
    for (int j = 0; j < Nsubsteps_; ++j) {
      ...
      // navierstokesNL(un, ubase_, Ubase_, fj_, tmp_, tmp2_,
      // flags_.nonlinearity);
      nse_->nonlinear(fields, fj_);
  ...
  //   cfl_ = fields[0].CFLfactor (Ubase_, Wbase_);         //calculated on
  //   demand through DNS class cfl_ *= flags_.dealias_xz()
  //   ? 2.0*pi/3.0*flags_.dt : pi*flags_.dt;
